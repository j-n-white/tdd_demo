...
class RomanNumeralsTest extends AnyPropSpec with TableDrivenPropertyChecks {
  property("Given the toRomanNumerals function when called with an integer then it returns the correct string") {
  ...
  property("Given the toRomanNumerals function when called with an out of range integer then it throws an error") {
